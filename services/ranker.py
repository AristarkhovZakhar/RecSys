...
class Ranker:
    def __init__(self, tau=0.7, model_name: str = 'all-MiniLM-L6-v2'):
        ...
        self.labels = Variable.get('LABELS', default_var=
        variable['http://ginger-news.ru:8000/documents/24caa543-e370-4eaf-97bb-02465b8fd6e6']['labels'])
        self.tau = tau
        self.model = SentenceTransformer(model_name)
        self.scores = {k: v for k, v in
                       json.loads(Variable.get('RANKER_SCORES', default_var=None)).items()} \
                      or {label: 1 for label in self.labels}
        self.counts = {k: v for k, v in
                       json.loads(Variable.get('RANKER_COUNTS', default_var=None)).items()} \
                      or {label: 1 for label in self.labels}
    ...
    def run_ranking_push_poster(self, **kwargs):
        ...
        messages = self.get_valid_news(summaries, variable)
